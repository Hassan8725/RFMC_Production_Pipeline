...
''' Section 2.2 Get Unique BTNs for each day Function '''
...
''' Section 3 Main Pipeline for Preparing and Updating Lookup '''

process_date=dt.datetime.now()
df1=get_data(process_date)
rfmc=get_rfmc(df1)

rfmc_final=rfmc.copy()
rfmc_final.drop(['Recency', 'Frequency', 'MonetaryValue', 'Clumpiness','pdate'],inplace=True,axis=1)
rfmc_final.set_index('customerid',inplace=True)
...
rfmc_final.to_sql('rfmc_lookup', con=engine, if_exists='append')
print("Rows Inserted in the MySQL Server Lookup Table:\n", rfmc_final.shape)

print("\nTaken taken for the whole process: ",dt.datetime.now()-process_date)